...
class IWAE:
    ...
    def __create__placeholder(self):
        with tf.name_scope("data"):
            self.data_noise = tf.random_normal([self.batch_size, 20], name='data_noise')
            self.data_w1 = tf.get_variable('data_w1', [20, 200], tf.float32,
                tf.contrib.layers.xavier_initializer(), trainable=False)
            self.data_b1 = tf.Variable(tf.zeros([200]), trainable=False, name='data_b1')
            self.data_h1 = tf.matmul(self.data_noise, self.data_w1) + self.data_b1
            self.data_h1_activate = tf.nn.relu(self.data_h1)
            self.data_w2 = tf.get_variable('data_w2', [200, 200], tf.float32,
                tf.contrib.layers.xavier_initializer(), trainable=False)
            self.data_b2 = tf.Variable(tf.zeros([200]), trainable=False, name='data_b2')
            self.data_h2 = tf.matmul(self.data_h1_activate, self.data_w2) + self.data_b2
            self.data_h2_activate = tf.nn.relu(self.data_h2)
            self.data_w3 = tf.get_variable('data_w3', [200, 400], tf.float32,
                tf.contrib.layers.xavier_initializer(), trainable=False)
            self.data_b3 = tf.Variable(tf.zeros([400]), trainable=False, name='data_b3')
            self.x = tf.matmul(self.data_h2_activate, self.data_w3) + self.data_b3
