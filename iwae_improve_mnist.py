...
class IWAE_MNIST:
    ...
    def __create_loss(self):
        ...
        with tf.name_scope("posterior_loss"):
            self.fz = tf.stop_gradient(self.logit, 'fz')
            self.fz_max = tf.stop_gradient(self.logit_max, 'fz_max')

            self.noise_posterior = tf.random_normal([self.batch_size, self.latent_dim], name='noise_posterior')
            self.z_posterior = tf.multiply(self.sd_z, self.noise_posterior) + self.mu_z
            self.decoder_posterior = []
            self.decoder_activate_posterior = []
            self.previous_tensor = self.z_posterior
            self.previous_dim = self.latent_dim
            for i_hidden in range(self.num_hidden):
                self.decoder_posterior.append(tf.matmul(self.previous_tensor, self.decoder_w[i_hidden]) + self.decoder_b[i_hidden])
                self.decoder_activate_posterior.append(tf.nn.tanh(self.decoder_posterior[i_hidden]))
                self.previous_tensor = self.decoder_activate_posterior[i_hidden]
                self.previous_dim = self.hidden_dim
            self.x_hat_posterior = tf.nn.sigmoid(tf.matmul(self.previous_tensor, self.x_hat_w) + self.x_hat_b)
            self.logit_z_posterior = tf.log(self.sd_z + self.protector) + tf.square(self.noise_posterior) / 2 - tf.square(self.z_posterior) / 2
            self.logit_x_posterior = tf.multiply(self.x, tf.log(self.x_hat_posterior + self.protector)) + tf.multiply(1 - self.x, tf.log(1 - self.x_hat_posterior + self.protector))
            self.fz_posterior = tf.reduce_sum(self.logit_z_posterior, 1) + tf.reduce_sum(self.logit_x_posterior, 1)
            self.fz_max_all = tf.reduce_max(tf.stack([self.fz_max, self.fz_posterior], axis=1), axis=1)
            self.fz_max_all_tile = tf.tile(tf.reshape(self.fz_max_all, [self.batch_size, 1]), [1, self.k])
            self.res_all = tf.reduce_sum(tf.exp(self.fz - self.fz_max_all_tile), 1)
            self.posterior_loss = tf.reduce_sum(-self.fz_posterior + self.fz_max_all + tf.log(tf.exp(self.fz_posterior - self.fz_max_all) + self.res_all)) / self.batch_size
        with tf.name_scope("joint_loss"):
            self.joint_loss = self.loss + self.posterior_loss * 0.0
            self.fz_posterior_gradient = tf.gradients(self.joint_loss, self.fz_posterior)

    def __create_optimizer(self):
        with tf.name_scope("optimizer"):
            self.optimizer = tf.train.AdamOptimizer(learning_rate=self.learning_rate).minimize(self.joint_loss)
    ...
    def __create_summary(self):
        with tf.name_scope("summary"):
            ...
            self.summary_posterior = tf.summary.scalar('posterior_loss', self.posterior_loss)
            self.summary_joint = tf.summary.scalar('joint_loss', self.joint_loss)
            self.summary_classifier = tf.summary.scalar('classifier_loss', self.classifier_loss)
            self.summary_all_iwae = tf.summary.merge([self.summary_iwae, self.summary_posterior, self.summary_joint])
    ...
    with tf.Session() as sess:
        ...
        for index in range(initial_step, int(finish_step)):
            ...
            if end_idx >= x.shape[0]:
                start_idx = 0
                end_idx = start_idx + model.batch_size
            g, batch_loss, _, summary = sess.run([model.posterior_loss, model.loss, model.optimizer, model.summary_all_iwae], feed_dict=feed_dict)
